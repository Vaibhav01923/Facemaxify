...
export const SkincareTimeline: React.FC<SkincareTimelineProps> = ({ 
  ...
  // Simplification: Just show the analysis for the CURRENT scan (or passed analysis)
  // No timeline, no upload button.
  
  const analysis = scans.find(s => s.id === currentScanId)?.analysis?.skincare;
  ...
  if (!analysis) {
      return (
        <div className="flex flex-col items-center justify-center p-12 text-center border border-white/5 rounded-3xl bg-slate-900/30">
            ...
            <h3 className="text-xl font-bold text-white mb-2">No Skincare Analysis</h3>
