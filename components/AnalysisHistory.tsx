...
export const AnalysisHistory: React.FC<AnalysisHistoryProps> = ({
  ...
  const { user } = useUser();
  // Removed internal state and fetching effect as controlled by parent
      ...
      <div className="p-4 border-b border-white/5">
          ...
          {history.length > 0 && isPaid && (
            <div className="text-right">
              ...
              <div className="text-2xl font-black text-white tracking-tighter leading-none">
                {Math.round(
                  (history.find((s) => s.id === selectedScanId) || history[0])
                    .overall_score * 10,
                )}
              </div>
            </div>
        ...
        {!isPaid && (
          <button
            ...
            onClick={() =>
              window.open(
                "https://buy.stripe.com/test_8wM6qC6d83U82Zy144",
                "_blank",
              )
            }
          >
                ...
                {isPaid && (
                  <span
                    className={`text-xs font-black ${
                      scan.overall_score * 10 >= 80
                        ? "text-emerald-400"
                        : scan.overall_score * 10 >= 60
                          ? "text-indigo-400"
                          : "text-slate-400"
                    }`}
                  >
