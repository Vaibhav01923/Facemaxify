  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        const leftEye = landmarks['leftEyeLateralCanthus'] || landmarks['leftEyePupil'];
        const rightEye = landmarks['rightEyeLateralCanthus'] || landmarks['rightEyePupil'];
        ...
        if (hairline && chin) {
            ...
            faceHeightPixels = Math.sqrt(Math.pow(chx - hx, 2) + Math.pow(chy - hy, 2));
        } else if (chin) {
            // Estimate: Eye to Chin is roughly 60% of face height? 
            // Let's rely on eye-to-chin distance * 1.6
            const chx = (chin.x / 1000) * w;
            const chy = (chin.y / 1000) * h;
            const eyeToChin = Math.sqrt(Math.pow(chx - cx, 2) + Math.pow(chy - cy, 2));
