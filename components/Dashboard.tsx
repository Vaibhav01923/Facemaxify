...
import { updateScanLandmarks, updateScanAnalysis, calculateWebsitePercentile, getScanHistory } from "../services/supabase";
import { getAiRecommendations, generateSkincareRoutine } from "../services/aiService";
...
export const Dashboard: React.FC<DashboardProps> = ({ data, isPaid = false, scanId }) => {
  ...
  const [activeTab, setActiveTabState] = useState<"overview" | "front" | "side" | "skincare">(initialTab);
      ...
      {!isPaid && (
        <div className="bg-indigo-600 px-4 py-2 text-center relative z-50 overflow-hidden shadow-lg shadow-indigo-500/20">
          ...
          <p className="relative z-10 text-[11px] sm:text-xs font-black text-white uppercase tracking-widest flex items-center justify-center gap-3">
            ...
            <button 
              onClick={() => window.location.href = `/api/checkout?customerEmail=${user?.primaryEmailAddress?.emailAddress}&discountCode=EARLY40`}
